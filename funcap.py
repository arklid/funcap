...
class FunCapHook(DBG_Hooks):
    ...
    def get_num_args_stack(self, addr):
        ...
        argFrameSize = (
            get_struc_size(get_func_attr(addr, FUNCATTR_FRAME))
            - get_frame_size(addr)
    ...
    def format_normal(self, regs):
        ...
        for reg in regs:
            ...
            if function_name and function_name != "N/A":
                ...
                if self.hexdump:
                    valchain_full_left = (
                        self.HEXMODE_LENGTH - len(valchain_full) - 1
                    ) / 4
                    valchain_cmt_left = (
                        self.HEXMODE_LENGTH_IN_COMMENTS - len(valchain_cmt) - 1
                    ) / 4
                    format_string_dump = " (%s)"
                else:
                    valchain_full_left = self.STRING_LENGTH - len(valchain_full)
                    valchain_cmt_left = self.STRING_LENGTH_IN_COMMENTS - len(
                        valchain_cmt
                    )
                    format_string_dump = ' ("%s")'
                ...
                valchain_full += format_string_dump % self.smart_format(
                    self.dereference(prev_memval, 2 * valchain_full_left),
                    valchain_full_left,
                )
                valchain_cmt += format_string_dump % self.smart_format_cmt(
                    self.dereference(prev_memval, 2 * valchain_cmt_left),
                    valchain_cmt_left,
                )
    ...
    def handle_function_end(self, ea):
        ...
        if function_name:
            header = (
                "At function end: %s (0x%x) " % (function_name, ea) + "to " + caller
            )
    ...
    def handle_return(self, ea):
        ...
        if sp in self.saved_contexts:
            ...
            self.output(
                "WARNING: saved context not found for stack pointer 0x%x, assuming function %s"
    ...
    def handle_function_start(self, ea):
        ...
        header = (
            "At function start: %s (0x%x) " % (name, ea)
            + "called by %s" % caller_offset
        )
        ...
        self.calls_graph[ea]["callers"].append(
            {"name": caller_name, "ea": caller_ea, "offset": caller_offset}
        )
    ...
    def handle_call(self, ea):
        ...
        if (
            self.current_caller
        ):  # delayed_caller: needed if breakpoint hits after signle step request
            self.delayed_caller = {
                "type": "call",
                "addr": ea,
                "ctx": self.get_context(ea=ea, depth=0),
            }
        else:
            self.current_caller = {
                "type": "call",
    ...
    def handle_after_jump(self, ea):
        ...
        if (
            self.code_discovery
            and (not is_code(get_full_flags(ea)) or not self.isCode)
            and not self.is_system_lib(seg_name)
        ):
            self.output(
                "New code segment discovered: %s (0x%x => 0x%x)"
    ...
    def discover_function(self, ea):
        ...
        if (
            ...
            and not re.match("loc_", symbol_name)
        ):
            self.output(
                "WARNING: IDA has probably wrongly analyzed the following function: %s and "
                "it is overlapping with another symbol: %s. Funcap will undefine it. "
                % (name, symbol_name)
            )
        ...
        if need_hooking:
            ...
            refresh_debugger_memory()
            self.output(
                "0x%x: new code section detected: [0x%x, 0x%x]" % (ea, start_ea, end_ea)
            )
            plan_and_wait(start_ea, end_ea)
    ...
    def handle_after_call(self, ret_addr, stub_name):
        ...
        if self.is_fake_call(ea):
            set_cmt(
                self.current_caller["addr"], "fake function call to 0x%x" % ea, False
            )
            self.output(
                "0x%X: fake function call to 0x%x" % (self.current_caller["addr"], ea)
            )
            self.current_caller = self.delayed_caller
            self.delayed_caller = None
            return 0
        ...
        if name:
            ...
            if (
                (self.recursive or self.code_discovery)
                and not self.is_system_lib(seg_name)
                and name not in self.hooked
            ):
                self.hookFunc(func=name)
        else:
            name = get_name(
                ea, ida_name.GN_VISIBLE
            )  # maybe there is a symbol (happens sometimes when the IDA analysis goes wrong)
            if not name:
                name = "0x%x" % ea
        ...
        if self.stub_name:
            header = (
                "Function call: %s to %s (0x%x)" % (caller, stub_name, ea)
        ...
        if ea not in self.calls_graph:
            ...
            self.calls_graph[ea]["callers"] = []
        self.calls_graph[ea]["callers"].append(
            {"name": caller_name, "ea": caller_ea, "offset": caller}
        )
        self.calls_graph[ea]["name"] = name
    ...
    def is_system_lib(self, name):
        ...
        sysfolders = [
            re.compile("\\\\windows\\\\", re.I),
            re.compile("\\\\Program Files ", re.I),
            re.compile("/usr/", re.I),
            re.compile("/system/", re.I),
            re.compile("/lib/", re.I),
        ]
    ...
    def dbg_bpt(self, tid, ea):
        ...
        if ea in list(
            self.function_calls.keys()
        ):  # coming back from a call we previously stopped on
    ...
    def dbg_step_into(self):
        ...
        if (
            hasattr(self, "current_caller")
            and self.current_caller
            and self.current_caller["type"] == "jump"
        ):
            ...
            if (
                hasattr(self, "current_caller")
                and self.current_caller
                and ret_addr == self.next_ins(self.current_caller["addr"])
            ):
                ...
                self.delayed_caller = None
                if self.resume:
                    "FunCap: unexpected single step"  # happens sometimes - due to a BUG in IDA. Hope one day it will be corrected
        if self.resume:
            request_continue_process()
            run_requests()
        return 0
...
class X86CapHook(FunCapHook):
    ...
    def get_context(self, general_only=True, ea=None, depth=None, stack_offset=1):
        ...
        if ea != None or depth != None:
            regs = regs + self.get_stack_args(
                ea, depth=depth, stack_offset=stack_offset
            )
        return regs
    ...
    def check_stub(self, ea):
        ...
        if (
            dbytes == "\x8b\xff\x55\x8b\xec\x5d\xe9"
            or dbytes == "\x8b\xff\x55\x8b\xec\x5d\xeb"
        ):
            return 5
...
class AMD64CapHook(FunCapHook):
    ...
    def __init__(self, **kwargs):
        ...
        self.CMT_CALL_CTX = [
            ...
            re.compile("^R8"),
            re.compile("^R9"),
        ]  # we are capturing 4 args, but it can be extended
        self.CMT_RET_SAVED_CTX = [
            re.compile("^RCX"),
            re.compile("^RDX"),
            re.compile("^R8"),
            re.compile("^R9"),
...
class CallGraph(GraphViewer):
    ...
    def OnRefresh(self):
        ...
        for hit, call in self.calls.items():
            ...
            name = call["name"]
            current_name = get_func_name(
                hit
            )  # check if the user has changed the name of a function
            if current_name:
                name = current_name
...
class Auto:
    ...
    def win_code_discovery(self):
        ...
        del_bpt(start)
        d.addStop(
            get_name_ea_simple("kernel32_ExitProcess")
        )  # last breakpoint before the process terminates, to give a chance to take a memory snapshot
        d.addStop(
            get_name_ea_simple("ntdll_RtlExitUserProcess")
        )  # to be sure we add 2 possibilities, there is probably more ...
        d.on()
